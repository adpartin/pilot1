...
def break_src_data(
        data: pd.DataFrame,
        target: str='AUC', # TODO: consider to assign None
        scaler: str='stnd',
    ...
    if target is not None:
        data = data[~data[target].isna()] # drop data points where target is NaN 
        ydata = data[target]
    else:
        ydata = None
    ...
    if target_transform and target is not None:
    ...
    # Get xdata and meta 
    xdata, meta = split_fea_and_other_cols(data, fea_prfx_dict)
...
def extract_subset_fea(data, fea_list: list, fea_sep: str='_'):
    ...
    Returns:
        data : updated data
    """
    fea_data, other_data = split_fea_and_other_cols(data, fea_prfx_dict)
...
def split_fea_and_other_cols(data, fea_prfx_dict: dict):
...
def print_fea_shapes(df, logger=None):
    ...
    """
    for prfx in np.unique(list(map(lambda x: x.split('_')[0], df.columns.tolist()))):
